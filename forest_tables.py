...
RoutingPGExpressions = PGExpressions(
    ...
    sweep_expired_destinations="UPDATE {self.table} SET experation_ms=NULL status='available' WHERE expiration_ms IS NOT NULL AND expiration_ms < (extract(epoch from now()) * 1000);",
