#!/usr/bi:n/python3.9
# pylint: disable=too-many-branches disable=too-many-return-statements disable=line-too-long
# pylint: disable=too-many-arguments disable=too-many-nested-blocks disable=too-many-locals
# pylint: disable=too-many-lines disable=unused-argument disable=too-many-public-methods
"""Executable file which starts up a bot which does mobilecoin airdrops"""
import logging
import json
import asyncio
from datetime import datetime
from enum import Enum
from typing import Any, Union, Optional, Type, TypeVar
from dataclasses import dataclass, field, asdict
from asyncio import create_task
from aiohttp import web
import mc_util
from decimal import Decimal
from forest.core import PayBot, app, Message, Response, requires_admin, hide
from forest.utils import get_secret
from forest.pghelp import Loop, PGExpressions, PGInterface
from forest.payments_monitor import delete_indices

T = TypeVar("T", bound="SimpleAirdrop")  # pylint: disable=invalid-name

FEE_MOB = 0.0004
FEE_PMOB = 400000000
SIGNAL_PAY = "go to Signal Settings > Payments > Activate Payments > Accept and Agree to Terms to activate signal payments!"

AirdropPGExpressions = PGExpressions(
    table="airdrops",
    create_table="CREATE TABLE airdrops ( \
            id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \
            drop_id text UNIQUE NOT NULL, \
            data jsonb, \
            drop_config jsonb, \
            created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL, \
            updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL, \
            drop_type text NOT NULL, \
            start_block integer NOT NULL);",
    initialize_defaults="INSERT INTO airdrops (drop_id, start_block, drop_type, data \
            VALUES ('no_airdrop', 0, 'none', '{{\\\"tx_list\\\":[]}}') ON \
            CONFLICT DO NOTHING;",
    create_simple_airdrop="INSERT INTO airdrops (drop_id, start_block, drop_type, data, drop_config) \
            VALUES ($1, $2, $3, '{{\\\"tx_list\\\":[]}}', $4);",
    sd_get_config="SELECT drop_config from airdrops WHERE drop_id = $1;",
    sd_update_config="UPDATE airdrops SET drop_config = drop_config || $2 WHERE drop_id = $1;",
    sd_get_last_airdrop="SELECT * from airdrops ORDER BY created_at desc limit 1;",
    sd_add_tx="UPDATE airdrops SET data = jsonb_set(data,'{{\\\"tx_list\\\", \
            99999999}}', $2, TRUE) WHERE drop_id = $1 ;",
    sd_filter_entrants="SELECT username, wallet_address, tx_block, bot_number \
            FROM airdrops,jsonb_to_recordset(airdrops.data->'tx_list')  AS (username text, \
            wallet_address text, tx_block integer, tx_purpose text, bot_number text) \
            WHERE drop_id = $1 and tx_purpose = $2 AND username NOT IN \
            (SELECT username from airdrops, \
            jsonb_to_recordset(airdrops.data->'tx_list') as (username text, tx_purpose \
            text) WHERE drop_id=$1 AND tx_purpose= $3);",
    sd_get_paid_entrants="SELECT username, wallet_address, tx_block, bot_number\
            FROM airdrops,jsonb_to_recordset(airdrops.data->'tx_list') \
            AS (username text, wallet_address text, tx_block integer, tx_purpose text, bot_number text) \
            WHERE drop_id = $1 AND tx_purpose = 'airdrop_payout'",
    sd_get_all_transactions="SELECT username, wallet_address, amount direction, \
            tx_purpose, tx_block, bot_number \
            FROM airdrops,jsonb_to_recordset(airdrops.data->'tx_list') \
            AS (username text, wallet_address text, amount numeric, direction \
            text, tx_purpose text, tx_block integer, bot_number text) \
            WHERE drop_id = $1 and tx_block > start_block;",
)


class AirdropManager(PGInterface):
    "Airdrop database ORM"

    def __init__(
        self,
        queries: PGExpressions = AirdropPGExpressions,
        database: str = get_secret("DATABASE_URL"),
        loop: Loop = None,
    ) -> None:
        super().__init__(queries, database, loop)


def try_cast_float(value: Any) -> Union[float, Any]:
    """
    Attempt to cast to float
    """
    if isinstance(value, bool):
        logging.warning(
            "bool value passed, aborting typecast to avoid undesired casting errors"
        )
        return value
    try:
        fvalue = float(value)
        return fvalue
    except Exception:  # pylint: disable=broad-except
        return value


def try_cast_int(value: Any) -> Union[int, Any]:
    """
    Attempt to cast to int
    """
    if isinstance(value, (float, bool)):
        logging.warning(
            "float or bool value passed, aborting typecast to avoid undesired casting errors"
        )
        return value
    try:
        ivalue = int(value)
        return ivalue
    except Exception:  # pylint: disable=broad-except
        return value


class States(Enum):
    """
    Possible environment states. States are not mutually exclusive
    """

    SETUP = 1
    NEEDS_FUNDING = 2
    READY_TO_LAUNCH = 3
    LIVE = 4
    AIRDROP_FULL = 5
    AIRDROP_FINISHED = 6
    NO_AIRDROP = 7
    ERROR = 8


class Dialog(Enum):
    """
    Possible dialogs
    """

    NONE = 1
    ADMIN_AIRDROP_SETUP = 2
    ADMIN_DISTRIBUTE_MOBILECOIN = 3


@dataclass
class Airdrop:
    """
    Base airdrop configuration type
    """

    name: str = "no_airdrop"
    # pylint: disable=R0201
    def is_configured_correctly(self) -> bool:
        """
        Is the airdrop configured correctly, alwayas false for base airdrop type
        """
        return False


@dataclass
class SimpleAirdrop(Airdrop):  # pylint: disable=too-many-instance-attributes
    """
    Configuration meant to represent an airdrop that distributes an equal
    amount to each participant
    """

    name: str = "no_airdrop"
    entry_price: float = -1.0
    drop_amount: float = -1.0
    max_entrants: int = -1
    start_block: int = -1
    dialog: Dialog = Dialog.NONE
    end_state: str = "unfinished"
    setup_script: dict = field(default_factory=dict)
    drop_script: dict = field(default_factory=dict)
    dialog_state: dict = field(default_factory=dict)

    def __post_init__(self) -> None:
        logging.info("Instantiating airdrop configuration, type: simple")
        self.setup_script = dict(
            welcome="Welcome to airdrop setup, I will now ask a series of questions to setup "
            "your drop. You may exit setup by typing 'exit' at anytime",
            entry_price=(
                "What price in MoB should users pay to enter the airdrop? "
                "If you'd like it to be free to enter, enter 0.\n\nOtherwise enter a "
                "price in MoB and this amount will be pre-dropped to users. "
                "They will be instructed send this amount back to the bot "
                "to enter the airdrop in order to help familiarize them with payments."
            ),
            entry_price_invalid="Entry price must be a 0 or positive amount (such as 1, 0.1, 0,001, etc.), please re-enter",
            drop_amount="How many MoB will be given out to each entrant when the airdrop starts? (enter integer or decimal)",
            drop_amount_invalid="Drop amount must be a positive number, please re-enter",
            max_entrants="How many entrants total are allowed? (enter integer)",
            max_entrants_invalid="The amount of entrants must be specified as a positive whole number, please re-enter",
            setup_finished="Airdrop setup complete! Type /launch_airdrop to begin the airdrop",
            exit="Exiting setup. You may return to this dialog anytime by typing /setup_airdrop or cancel setup with /cancel_aidrop",
        )
        self.drop_script = dict(
            welcome="You've initiated a drop. You can type exit at anytime to cancel. Drop options:\n",
            options=(
                "You've initiated a drop. You can type exit at anytime to "
                "cancel.\n{num_users} unpaid users fully registred for airdrop\n\n"
                "Drop Options:\n"
                "1. Distribute {drop_amount} MoB to all {num_users} entrants\n"
                "2. Choose how many entrants to drop {drop_amount} MoB to\n"
            ),
            options_invalid="Input invalid, enter 1 or 2 to choose an option",
            specify_amount="Enter # of entrants to make drop to, {num_users} unpaid airdrop entrants remaining",  # pylint: disable=line-too-long
            specify_amount_invalid="Input invalid, {num_users} users entered, enter a number of users equal to or below {num_users}",
            confirm_airdrop="Type 'confirm' to drop {amt} MoB to {num_users} users",
            confirm_airdrop_invalid="Invalid input, type confirm to make drop or exit to cancel",
            confirmed="Attemping to airdrop {amt} mob to {num_users} entrants",
            exit="Cancelling mob distribution, you can re-enter this dialog using /make_drop",
        )
        self.dialog_state = {}

    def is_configured_correctly(self) -> bool:
        """
        Checks for valid values for Simple Airdrop config type

        Returns:
          bool: whether setup for airdrop is complete
        """
        return self.entry_price >= 0 and self.drop_amount > 0 and self.max_entrants > 0

    def end_dialog(self) -> None:
        """
        Clear all dialog artifacts
        """
        self.dialog_state = {}
        logging.info("Exiting dialog %s", self.dialog.name)
        self.dialog = Dialog.NONE

    def enter_dialog(self, dialog: Dialog) -> None:
        """
        Enter new dialog, clear old dialog artifacts

        args:
          dialog (Dialog): dialog: type of dialog to enter
        """
        self.dialog_state = {}
        if not dialog in Dialog:
            logging.warning("Invalid dialog specified, no dialog started")
            return
        logging.info("Entering dialog %s", dialog.name)
        self.dialog = dialog

    def set_airdrop(self, block: int, name: str) -> None:
        """
        Set block airdrop will start at. Once this is set, bot will consider
        airdrop launched.

        args:
          block (int): Block to start airdrop at
          name (str): Name of the airdrop
        """
        if self.start_block >= 0:
            logging.info("Airdrop in process, please clear airdrop before proceeding")
            return
        logging.info("setting Airdrop start at block %s", block)
        self.start_block = block
        self.name = name

    def clear_airdrop(self) -> None:
        """
        Clear airdrop configuration, this ends the airdrop
        """
        logging.info("Resetting airdrop to default values")
        self.name = "simple_airdrop_setup"
        self.dialog = Dialog.NONE
        self.dialog_state = {}
        self.set_airdrop(-1, "no_airdrop")
        self.max_entrants = -1
        self.entry_price = -1.0
        self.drop_amount = -1.0

    def get_next_dialog(  # pylint: disable=too-many-branches disable=too-many-return-statements
        self, invalid: bool = False, data: Optional[dict] = None
    ) -> str:
        """
        Gets next dialog in setup sequence based on current configuration

        args:
          invalid (bool): return dialog that responds to invalid input
          data (Optional[dict]): associated metadata needed for dialog

        Returns:
          str: next setup dialog
        """
        valid = ""
        if invalid:
            valid = "_invalid"

        if self.dialog == Dialog.ADMIN_AIRDROP_SETUP:
            if self.entry_price < 0:
                return (
                    self.setup_script["welcome"]
                    + "\n\n"
                    + self.setup_script["entry_price" + valid]
                )
            if self.drop_amount < 0:
                return self.setup_script["drop_amount" + valid]
            if self.max_entrants < 1:
                return self.setup_script["max_entrants" + valid]
            self.end_dialog()
            return self.setup_script["setup_finished"]

        if (
            self.dialog == Dialog.ADMIN_DISTRIBUTE_MOBILECOIN
        ):  # pylint: disable=too-many-nested-blocks
            if isinstance(data, dict):
                unpaid_entrants = data.get("num_unpaid_entrants")
                selection = self.dialog_state.get("selection")
                num_to_drop = self.dialog_state.get("num_to_drop")
                confirmed = self.dialog_state.get("confirmed")
                if not selection and not confirmed:
                    return self.drop_script["options" + valid].format(
                        num_users=unpaid_entrants, drop_amount=self.drop_amount
                    )
                if selection:
                    if selection == 1:
                        if confirmed:
                            return self.drop_script["confirmed"].format(
                                num_users=unpaid_entrants, amt=self.drop_amount
                            )
                        return self.drop_script["confirm_airdrop" + valid].format(
                            num_users=unpaid_entrants, amt=self.drop_amount
                        )
                    if selection == 2:
                        if num_to_drop:
                            if confirmed:
                                return self.drop_script["confirmed"].format(
                                    num_users=num_to_drop, amt=self.drop_amount
                                )
                            return self.drop_script["confirm_airdrop" + valid].format(
                                num_users=num_to_drop, amt=self.drop_amount
                            )
                        return self.drop_script["specify_amount" + valid].format(
                            num_users=unpaid_entrants
                        )
        self.end_dialog()
        return ""

    def take_next_input(
        self, value: Any, data: dict
    ) -> str:  # pylint: disable=too-many-return-statements
        """
        Take input via a dialog flow

        args:
          value (Any): Value input during dialog flow

        Returns:
          str: reply string
        """
        if self.dialog == Dialog.ADMIN_AIRDROP_SETUP:
            logging.info("airdrop state is %s", repr(self))
            if value == "exit":
                self.end_dialog()
                return self.setup_script.get("exit", "Exiting dialog")
            if self.entry_price < 0:
                value = try_cast_float(value)
                if isinstance(value, (float, int)) and (value >= 0):
                    self.entry_price = value
                    return self.get_next_dialog()
                return self.get_next_dialog(invalid=True)
            if self.drop_amount < 0:
                value = try_cast_float(value)
                if isinstance(value, (float, int)) and (value > 0):
                    self.drop_amount = value
                    return self.get_next_dialog()
                return self.get_next_dialog(invalid=True)
            if self.max_entrants < 1:
                value = try_cast_int(value)
                if isinstance(value, (int)) and (value > 0):
                    self.max_entrants = value
                    return self.get_next_dialog()
                return self.get_next_dialog(invalid=True)
            return self.get_next_dialog()

        if self.dialog == Dialog.ADMIN_DISTRIBUTE_MOBILECOIN:
            unpaid_entrants = data.get("num_unpaid_entrants")
            selection = self.dialog_state.get("selection")
            num_to_drop = self.dialog_state.get("num_to_drop")
            confirmed = self.dialog_state.get("confirmed")
            if value == "exit":
                self.end_dialog()
                return self.drop_script.get("exit", "Exiting dialog")
            if not selection:
                value = try_cast_int(value)
                if isinstance(value, int) and (
                    -1 < value <= 2
                ):  # pylint: disable=chained-comparison
                    self.dialog_state["selection"] = value
                    if value == 1:
                        self.dialog_state["num_to_drop"] = unpaid_entrants
                    return self.get_next_dialog(data=data)
                return self.get_next_dialog(invalid=True, data=data)
            if selection:
                if selection == 2:
                    num_to_drop = self.dialog_state.get("num_to_drop")
                    if not num_to_drop:
                        num_to_drop = try_cast_int(value)
                        if (
                            isinstance(num_to_drop, int)
                            and isinstance(unpaid_entrants, int)
                            and unpaid_entrants >= num_to_drop > 0
                        ):
                            self.dialog_state["num_to_drop"] = num_to_drop
                            return self.get_next_dialog(data=data)
                        return self.get_next_dialog(invalid=True, data=data)
                    if (
                        isinstance(num_to_drop, int)
                        and isinstance(unpaid_entrants, int)
                        and unpaid_entrants >= num_to_drop > 0
                    ):
                        if value == "confirm":
                            self.dialog_state["confirmed"] = True
                            logging.info(
                                "Airdrop of %s to %s entrants confirmed",
                                self.drop_amount,
                                num_to_drop,
                            )
                            return self.get_next_dialog(data=data)
                        return self.get_next_dialog(invalid=True, data=data)
                if selection == 1:
                    if confirmed:
                        return self.get_next_dialog(data=data)
                    if value == "confirm":
                        self.dialog_state["confirmed"] = True
                        logging.info(
                            "Airdrop of %s to %s entrants confirmed",
                            self.drop_amount,
                            unpaid_entrants,
                        )
                        return self.get_next_dialog(data=data)
                    return self.get_next_dialog(invalid=True, data=data)

        self.end_dialog()
        return ""

    def get_total_airdrop_amount(
        self, include_fees: bool = True, padding: float = 0.05
    ) -> float:
        """
        Calculate total airdrop pot

        args:
          include_fees (bool): return amount plus estimated fees
          padding (float): fraction of total pot that should be available to
          cover unexpected events

        Returns
          float: total mob to distribute in airdrop
        """
        total_drop = self.max_entrants * self.drop_amount
        if include_fees:

            return total_drop + self.max_entrants * (FEE_MOB * 3) + total_drop * padding
        return total_drop

    def is_ready_to_launch(self, balance: Union[float, int]) -> bool:
        """
        Check balance to determine if airdrop is ready to start

        args:
          balance (Union[float, int]): wallet balance in pmob

        Returns:
          bool: whether airdrop can start
        """

        cost = self.get_total_airdrop_amount(include_fees=True)
        enough_mob_available = balance > cost
        configured_correctly = self.is_configured_correctly()
        can_start = enough_mob_available and configured_correctly
        if not enough_mob_available:
            logging.warning(
                "Airdrop will cost %s but wallet only has %s", cost, balance
            )
        if not configured_correctly:
            logging.warning("Airdrop is not configured correctly")
        return can_start

    @staticmethod
    def to_pmob(amount: Union[float, int]) -> int:
        """
        Takes amount in mob, returns amount in pmob
        """

        return int(mc_util.mob2pmob(amount))

    @staticmethod
    def to_mob(amount: int) -> float:
        """
        Takes amount in pmob, returns amount in mob
        """

        return float(round(mc_util.pmob2mob(amount), 4))

    def asdict(self) -> dict:
        """
        Output dataclass as dict
        """

        return {
            k: v
            for k, v in asdict(self).items()
            if k
            in [
                "name",
                "entry_price",
                "drop_amount",
                "max_entrants",
                "start_block",
                "end_state",
            ]
        }

    @classmethod
    def from_dict(cls: Type[T], env: dict) -> T:
        return cls(
            **{
                k: v
                for k, v in env.items()
                if k
                in [
                    "name",
                    "entry_price",
                    "drop_amount",
                    "max_entrants",
                    "start_block",
                    "end_state",
                ]
            }
        )

    def set_end_state(self, state: str) -> None:
        self.end_state = state

    def __repr__(self) -> str:
        resp = (
            "\nAirdrop Configuration:\n"
            f"Airdrop Name: {self.name}\n"
            f"Start Block: {self.start_block}\n"
            f"Drop Amount: {self.drop_amount}\n"
            f"Max Entrants: {self.max_entrants}\n"
            f"Entry Price: {self.entry_price}\n"
            f"Dialog State: {self.dialog}"
        )
        if self.is_configured_correctly():
            total = self.get_total_airdrop_amount()
            resp += f"\nDrop Total + Fees: {round(total, 4)}"
        return resp


class AirDropBot(PayBot):
    """
    Bot which takes airdrop entrants and provides a drop!
    """

    def __init__(self) -> None:
        self.name = "MobDripper"
        self.config: Airdrop = Airdrop()
        self.entrant_list: dict = {}
        self.db_manager = AirdropManager()
        self.txo_pools: dict[str, dict[str, int]] = {"deadpool": {}}
        self.resource_lock = asyncio.Lock()
        super().__init__()

    #        asyncio.create_task(self.check_last_airdrop())

    @staticmethod
    def is_admin(msg: Message) -> bool:
        """
        Determine if message sender is admin
        """
        admin = get_secret("ADMIN")
        secondary = get_secret("SECONDARY_ADMIN")
        return msg.source == admin or (
            isinstance(secondary, str) and msg.source in secondary.split(",")
        )

    async def handle_message(self, message: Message) -> Response:
        if self.is_admin(message) and isinstance(self.config, SimpleAirdrop):
            if not (message.text or message.command) or message.group:
                return None
            if self.config.dialog == Dialog.ADMIN_AIRDROP_SETUP:
                logging.info("In Airdrop Setup")
                if message.command:
                    self.config.end_dialog()
                    return await super().handle_message(message)
                return self.config.take_next_input(message.text, {})
            if self.config.dialog == Dialog.ADMIN_DISTRIBUTE_MOBILECOIN:
                unpaid_entrants = self.get_entrants("unpaid")
                failed_entrants = self.get_entrants("failed")
                data = {
                    "num_unpaid_entrants": len(unpaid_entrants) + len(failed_entrants)
                }
                dialog = self.config.take_next_input(message.text, data)
                if self.config.dialog_state.get("confirmed") is True:
                    num_to_drop = self.config.dialog_state.get("num_to_drop", 0)
                    create_task(self.distribute_drop(num_to_drop))
                    self.config.end_dialog()
                return dialog
        if message.payment:
            return await self.process_payment(message)

        return await super().handle_message(message)

    async def do_help(self, message: Message) -> Response:
        return await self.default(message)

    @hide
    @requires_admin
    async def do_recover_airdrop(self, msg: Message) -> Response:
        """
        Recover Airdrop
        """
        create_task(
            self.send_message(msg.source, "Attempting to recover the last airdrop")
        )
        rec = await self.db_manager.sd_get_last_airdrop()
        drop_id = rec[0].get("drop_id")
        state = await self.get_state()
        if not (States.NO_AIRDROP in state or States.SETUP in state):
            return "Airdrop currently in progress, can't run recovery"

        cohorts = []
        for i in (
            ("prepay", "airdrop_entry", "prepaid"),
            ("airdrop_entry", "airdrop_payout", "unpaid"),
        ):
            cohorts += [
                (i[2], await self.db_manager.sd_filter_entrants(drop_id, i[0], i[1]))
            ]
        cohorts += [("paid", await self.db_manager.sd_get_paid_entrants(drop_id))]
        for cohort in cohorts:
            for record in cohort[1]:
                self.entrant_list[record.get("username")] = dict(record)
                self.entrant_list[record.get("username")]["status"] = cohort[0]

        record = await self.db_manager.sd_get_config(drop_id)
        config = SimpleAirdrop.from_dict(json.loads(dict(record[0])["drop_config"]))

        if isinstance(config, SimpleAirdrop):
            wallet_balance = await self.mobster.get_wallet_balance()
            if not config.is_ready_to_launch(wallet_balance):
                return "Not enough balance in the wallet to launch! Please fund"
            drop_txos = [config.to_pmob(config.drop_amount) + FEE_PMOB] * (
                len(self.entrant_list) - len(self.get_entrants("paid"))
            )
            async with self.resource_lock:
                await self.preallocate_txo_pool("drop", drop_txos)
            if config.entry_price:
                entry_txos = [config.to_pmob(config.entry_price) + FEE_PMOB] * (
                    config.max_entrants - len(self.entrant_list)
                )
                async with self.resource_lock:
                    await self.preallocate_txo_pool("entry", entry_txos)
        self.config = config
        return "Airdrop successfully restarted! You can now announce the airdrop"

    async def process_payment(self, message: Message) -> Response:
        """
        Handle refunding of payments to users legitimately entering airdrop or
        sending unsolicted payments
        """
        state = await self.get_state()
        refund_without_fees = "we are refunding your payment minus transaction fees!"
        refund_with_fees = "we are refunding your payment and transaction fees!"
        if not States.LIVE in state:
            if States.AIRDROP_FINISHED:
                create_task(self.return_payment(message, "airdrop_finished"))
                return f"The airdrop has finished {refund_without_fees}"
            create_task(self.return_payment(message, "not_live"))
            return f"No airdrop in progress, {refund_without_fees}"
        if States.AIRDROP_FULL in state and not self.entrant_list.get(message.source):
            create_task(self.return_payment(message, "airdrop_full"))
            return f"This airdrop has reached the maximum amount of entrants, {refund_without_fees}"
        if message.source in self.entrant_list and isinstance(
            self.config, SimpleAirdrop
        ):
            status = self.entrant_list.get(message.source, {}).get("status")
            if status == "prepaid":
                create_task(self.return_payment(message, "prepaid_airdrop_entry"))
                return "You've successfully entered the airdrop!"
            if status in ("unpaid", "failed", "paid"):
                create_task(self.return_payment(message, "duplicate_payment"))
                return f"You've already entered this airdrop, {refund_without_fees}"
        if isinstance(self.config, SimpleAirdrop) and not (
            message.source in self.entrant_list
        ):
            if message.text and "pay me" in message.text.lower():
                if (
                    self.entrant_list.get(message.source)
                    or not States.LIVE in state
                    or States.AIRDROP_FULL in state
                    or States.SETUP in state
                ):
                    return None
                if States.NEEDS_FUNDING in state or States.ERROR in state:
                    return "Hello! Airdrops are currently paused for maintenance, please check back soon"
                wallet_address = await self.get_address(message.source)
                if not wallet_address:
                    resp = (
                        f"You haven't yet enabled Signal Pay! To enable it {SIGNAL_PAY}"
                    )
                    if self.config.entry_price > 0:
                        resp += f"\n\nAfter you've done that, I'll send you the initial {self.config.entry_price} MoB!"
                    return resp
                reason = "prepay"
                resp = (
                    f"Okay! I'm sending you {self.config.entry_price + FEE_MOB} "
                    f"MoB, send me back {self.config.entry_price} MoB to enter the "
                    f"airdrop. Once entered, you will be dropped "
                    f"{self.config.drop_amount} MoB once the airdrop starts!"
                )
                if self.config.entry_price == 0:
                    reason = "prepaid_airdrop_entry"
                    resp = f"You've entered the airdrop! When the drop starts you'll be sent {self.config.drop_amount} MoB"
                create_task(
                    self.return_payment(message, reason, wallet_address=wallet_address)
                )
                return resp
            create_task(self.return_payment(message, "airdrop_entry"))
            return f"You've successfully entered the airdrop! Since you sent your own MoB {refund_with_fees}"
        create_task(self.return_payment(message, "Unexpected"))
        return f"Your payment was unexpected, {refund_without_fees}"

    async def return_payment(
        self, msg: Message, reason: str, fee: int = 400000000, **params: Any
    ) -> None:
        """
        Return payments sent to the bot to the original sender

        args:
          msg (Response): Message object sent to the bot
          reason (str): Reason payment is being sent back
        """
        state = await self.get_state()
        if isinstance(params.get("wallet_address"), str):
            wallet_address = params.get("wallet_address")
        else:
            wallet_address = await self.get_address(msg.source)
        if not isinstance(wallet_address, str):
            logging.info("could not get wallet address for %s", msg.source)
            self.send_message(
                msg.source,
                (
                    "we could not get your wallet address. "
                    f"if you don't have your wallet enabled {SIGNAL_PAY}\n\n"
                    f"If your wallet is enabled, please try again. "
                    "If this fails you can message airdrop support at "
                    f"{get_secret('SUPPORT')}"
                ),
            )
            return

        assert isinstance(wallet_address, str)
        entry_block = await self.mobster.get_current_network_block()
        amount_to_send = 0
        amount_received = 0
        payment_message = "Your payment has been refunded!"
        if reason == "prepay" and isinstance(self.config, SimpleAirdrop):
            amount_to_send = self.config.to_pmob(self.config.entry_price) + fee
            payment_message = f"I've sent you {self.config.to_mob(amount_to_send)} MoB"
            self.entrant_list[msg.source] = dict(
                status="prepaid",
                wallet_address=wallet_address,
                username=msg.source,
                tx_block=entry_block,
            )

        if msg.payment:
            logging.info("attempting to confirm payment from %s", msg.source)
            amount_received = await self.mobster.get_receipt_amount_pmob(msg.payment["receipt"])  # type: ignore
            if isinstance(amount_received, int):
                amount_to_send = amount_received - fee
                create_task(
                    self.record_tx(
                        msg.source,
                        reason,
                        amount_received,
                        "tx_direction_received",
                        wallet_address,
                        entry_block,
                    )
                )
            else:
                self.send_message(
                    msg.source,
                    (
                        "We weren't able to process your "
                        "payment. Transactions can sometimes fail, when that "
                        "happens the money you tried to send will be returned. "
                        "Check the transaction you sent in your signal wallet. "
                        "If the transaction shows a 'failed' status, try again as no "
                        "money has been debited from your account. If the "
                        "transaction was successfully sent and your account "
                        "shows the transaction was successful, please message "
                        f"{get_secret('SUPPORT')} for support"
                    ),
                )
                return

        if reason in ["airdrop_entry", "prepaid_airdrop_entry"]:
            if reason == "airdrop_entry":
                amount_to_send = amount_received + fee
            logging.info("%s entered airdrop, adding to list", msg.source)
            self.entrant_list[msg.source] = dict(
                status="unpaid",
                username=msg.source,
                wallet_address=wallet_address,
                tx_block=entry_block,
            )
            num_entrants = len(self.get_entrants("unpaid")) + len(
                self.get_entrants("failed")
            )
            if isinstance(self.config, SimpleAirdrop):
                if num_entrants >= self.config.max_entrants:
                    full_msg = f"Airdrop is full with {num_entrants}/{self.config.max_entrants} entrants, please make drop by typing /make_drop"
                    create_task(self.send_message(get_secret("ADMIN"), full_msg))

        if amount_to_send <= 0 or reason == "prepaid_airdrop_entry":
            create_task(
                self.record_tx(
                    msg.source,
                    "airdrop_entry",
                    0,
                    "tx_direction_sent",
                    wallet_address,
                    entry_block,
                )
            )
            return

        pool = "deadpool"
        if (
            not (States.AIRDROP_FULL in state)
            and (States.LIVE in state)
            and reason == "prepay"
        ):
            pool = "entry"
            assert isinstance(self.config, SimpleAirdrop)
            open_slots = self.config.max_entrants - len(self.entrant_list)
            if open_slots > 1:
                locked_txos: dict[str, Any] = {}
                for pool, utxos in self.txo_pools.items():
                    locked_txos.update(utxos)
                async with self.resource_lock:
                    if len(self.txo_pools.get(pool, ())) - open_slots < 0:
                        await self.preallocate_txo_pool(
                            pool, [amount_to_send] * (open_slots + 3)
                        )
            if len(self.entrant_list) >= self.config.max_entrants:
                resp = (
                    "Airdrop full, waiting for "
                    f"{len(self.get_entrants('prepaid'))} users to complete the "
                    "payment process. You can type /make_drop to make a drop to "
                    "users who are fully registered at anytime"
                )
                create_task(self.send_message(get_secret("ADMIN"), resp))

        async with self.resource_lock:
            input_txos = await self.get_txos_from_pool(pool, [amount_to_send])

        logging.info(
            "refunding %s MoB paid to %s reason: %s using input_txo_ids %s from pool %s",
            float(mc_util.pmob2mob(amount_to_send)),
            msg.source,
            reason,
            input_txos,
            pool,
        )

        retries = 0
        if reason == "prepay":
            retries = 2

        result = await self.send_payment(
            msg.source,
            amount_to_send,
            payment_message,
            30,
            comment=reason,
            input_txo_ids=list(input_txos.keys()),
        )

        if retries > 0 and result and result.status == "tx_status_failed":
            for i in range(retries):
                logging.warning("payment to %s failed, retry attempt %s", msg.source, i)
                async with self.resource_lock:
                    input_txos = await self.get_txos_from_pool(pool, [amount_to_send])
                result = await self.send_payment(
                    msg.source,
                    amount_to_send,
                    payment_message,
                    30,
                    comment=reason,
                    input_txo_ids=list(input_txos.keys()),
                )

        logging.info("Return payment result was %s", result)
        if result and result.status == "tx_status_succeeded":
            create_task(
                self.record_tx(
                    msg.source,
                    reason,
                    amount_to_send,
                    "tx_direction_sent",
                    wallet_address,
                    entry_block,
                )
            )

    async def do_signalme(self, _: Message) -> Response:
        """signalme
        Returns a link to share the bot with friends!"""
        return f"https://signal.me/#p/{self.bot_number}"

    @requires_admin
    async def do_balance(self, _: Message) -> Response:
        """Returns bot balance in MOB."""
        return f"Bot has balance of {mc_util.pmob2mob(await self.mobster.get_balance()).quantize(Decimal('1.0000'))} MOB"

    @hide
    @requires_admin
    async def do_pay(self, msg: Message) -> Response:
        if msg.tokens:
            payee = msg.tokens[0]
            amount = try_cast_float(msg.tokens[1])
        if not isinstance(amount, float):
            return "Sorry, the amount wasn't a number, try again with: /pay +signalnumber amount \nwith amount as a number"
        address = await self.get_address(payee)
        if not address:
            return "We couldn't get that number's address, please ensure the number is valid and the user has signal pay enabled"

        pmob = mc_util.mob2pmob(amount)
        async with self.resource_lock:
            inputs = await self.get_txos_from_pool("deadpool", [pmob])
        result = await self.send_payment(
            payee,
            pmob,
            f"I've just sent you {amount} MoB!",
            20,
            input_txo_ids=list(inputs.keys()),
        )
        if result.status == "tx_status_succeeded":
            return f"paid {payee} {amount} MoB!"
        return f"Payment to {payee} failed. Try again"

    async def default(self, message: Message) -> Response:
        if message.text and not (message.group or message.source == self.bot_number):
            conf = self.config
            resp = f"You've messaged {self.name}. \n\n"
            state = await self.get_state()
            logging.info("default reply to %s - state %s", message.source, state)
            if States.AIRDROP_FINISHED in state:
                if self.is_admin(message):
                    return "Hi admin, the airdrop is paid out in full, type /finish_airdrop to clean it up!"
                if message.source in self.get_entrants("paid", num_only=True):
                    return "Your MoB has been delivered! I appreciate you so much for participating, come back soon! "
                return "Hello there, the airdrop recently in progress has finished! Please check back later"
            if not isinstance(conf, SimpleAirdrop) or (States.SETUP in state):
                if self.is_admin(message):
                    if not isinstance(conf, SimpleAirdrop):
                        create_task(self.check_last_airdrop(message.source))
                        resp = (
                            "Hi admin, no airdrop is currently configured. Type "
                            "/setup_airdrop to configure one\n"
                        )
                        return resp
                    if not conf.is_configured_correctly():
                        resp = (
                            "Hi admin, an airdrop configuration is in progress. "
                            "Further configuration is needed, type /setup_airdrop "
                            "to enter airdrop setup\n"
                        )
                    else:
                        resp = "Hi admin, airdrop configuration is complete\n"
                    if States.NEEDS_FUNDING in state:
                        resp += (
                            "\nThe airdrop wallet does not have enough funds "
                            f"please add at least {conf.get_total_airdrop_amount()} "
                            "to the wallet before launching the airdrop\n"
                        )
                    if States.READY_TO_LAUNCH in state:
                        resp = (
                            "Hi admin, airdrop configuration is complete and "
                            "fully funded, you can launch it by typing /launch_airdrop\n"
                        )
                    resp += "\nYou can check airdrop configuration using /drop_stats\n"
                    resp += "You can cancel setup by typing /cancel_airdrop"
                    return resp
                resp += "Hello! No airdrop is in progress please check back later"
                return resp

            if States.LIVE in state or States.AIRDROP_FINISHED in state:
                if self.is_admin(message):
                    resp = "Hello admin, an airdrop is in progress.\n\n"
                    if States.NEEDS_FUNDING in state:
                        resp += "but has insufficient funding, PLEASE FUND IMMEDIATELY!"
                        return resp
                    if States.AIRDROP_FULL in state:
                        resp += "Airdrop is full!\n"
                    resp += "Your management options are:\n"
                    resp += "* Make the drop to current entrants by typing /make_drop\n"
                    resp += "* Message users with /blast [prepaid/entered/paid/all] [message]\n"
                    resp += "* Get airdrop stats by typing /drop_stats "
                    resp += "- add 'showusers' to show entered users\n"
                    resp += "* Message an individual user by typing /nessage [user] [message]\n\n"
                    resp += "Options to use with care!\n"
                    resp += "* Cancel the drop with /cancel_airdrop (you'll be asked to confirm first)\n"
                    resp += "* Pay ANY user with /pay [user] [amount]\n "
                    resp += "(don't use this except for support purposes, use /make_drop to payout airdrop users)\n\n"
                    resp += "Type any of these commands for further information"
                    return resp

                resp += "Hello! An airdrop is in progress. "
                if message.source in self.entrant_list:
                    status = self.entrant_list.get(message.source, {}).get("status")
                    if status == "unpaid":
                        resp += "You've already entered the Airdrop! You'll receive your MoB soon. "
                    if status == "prepaid":
                        resp += (
                            f"I've sent you {conf.entry_price + FEE_MOB} MoB! Please "
                            f"send {conf.entry_price} back and you will be entered "
                            "into the airdrop!"
                        )
                    if status == "paid":
                        resp = "Your MoB has been delivered! I appreciate you so much for participating, come back soon! "
                if not self.entrant_list.get(message.source):
                    if States.AIRDROP_FULL in state:
                        resp += "unfortunately the airdrop is full. Please check back later!"
                        return resp
                    if (
                        "pay me" in message.text.lower()
                        and not States.AIRDROP_FINISHED in state
                    ):
                        return await self.process_payment(message)

                    resp += (
                        f"I am currently dropping {conf.drop_amount} to "
                        f"each entrant for the first {conf.max_entrants} "
                        "entrants.\n\nTo enter this airdrop:\n1. Enable signal pay if "
                        "you have not already\n2. Message me 'pay me' and "
                    )
                    if conf.entry_price == 0:
                        resp += "you'll be entered into the Airdrop!\n\n"
                    else:
                        resp += (
                            f"I will send you {conf.entry_price + FEE_MOB} MoB\n"
                            f"3. Send me {conf.entry_price} MoB back and you'll be "
                            "entered into the airdrop!\n\n"
                        )

                    resp += (
                        "When the airdrop starts "
                        f"you'll be sent {conf.drop_amount} MOB! \n\n"
                        f"P.S. If you haven't enabled Signal Pay {SIGNAL_PAY}"
                    )
                return resp
        return None

    async def preallocate_txo_pool(
        self, name: str, txos: list[int], off_limits: Optional[dict] = None
    ) -> None:
        """
        Preallocate a list of txos

        args:
          name (str): txo pool to allocate into
          txos (list[int]): list of pmob valued amounts to turn into utxos
        """
        logging.info(
            "Attempting to pre allocate %s utxos into utxo pool %s", len(txos), name
        )
        off_limit_utxos = {}
        if isinstance(off_limits, dict):
            off_limit_utxos = off_limits.copy()
        for _, utxos in self.txo_pools.items():
            off_limit_utxos.update(utxos)
        account_id = await self.mobster.get_account()
        utxos = await self.mobster.preallocate_txos(
            account_id, txo_list=txos, locked_txos=off_limit_utxos
        )

        if name in self.txo_pools:
            self.txo_pools[name].update(utxos)
            return
        self.txo_pools[name] = utxos

    async def remove_spent_utxos(self, txos: dict[str, int]) -> dict[str, int]:
        """
        Remove txos from txo_pool list in place if they're already spent

        args:
          txos (dict): dict of txos to prune

        Returns
          (list): list of txo amounts removed
        """
        txos_removed = {}
        txo_ids = list(txos.keys())
        for txo_id in txo_ids:
            if not await self.mobster.is_txo_unspent(txo_id):
                logging.debug("%s is spent, removing", txo_id)
                txos_removed[txo_id] = txos.pop(txo_id)
        return txos_removed

    async def get_txos_from_pool(
        self, pool: str, txo_list: list[int]
    ) -> dict[str, int]:
        """
        Grab utxos from pool and validate they're available prior to spend

        args:
          pool (str): name of txo pool
          txo_list (list[int): list of amts to retrieve

        Returns:
          dict[str, int]: dictionary of utxos to use
        """
        logging.info(
            "Attemping to retrieve %s utxos from utxo pool %s", len(txo_list), pool
        )
        utxos: dict[str, int] = {}
        if not pool in self.txo_pools:
            logging.warning("txo pool '%s' does not exist", pool)
            return utxos

        remove_list: list[int] = []
        for i in range(len(txo_list)):  # pylint: disable=consider-using-enumerate
            if not self.txo_pools[pool]:
                break
            for txo_id, utxo_amt in self.txo_pools[pool].items():
                if utxo_amt >= txo_list[i]:
                    utxos[txo_id] = utxo_amt
                    remove_list.append(i)
                    self.txo_pools[pool].pop(txo_id)
                    break

        # Double check ensure txos are valid
        delete_indices(txo_list, remove_list)
        invalid = await self.remove_spent_utxos(utxos)
        for _, amt in invalid.items():
            txo_list.append(amt)

        if len(txo_list) > 0:
            if len(self.txo_pools[pool]) == 0:
                logging.info(
                    "txo pool %s is short %s utxos, allocating more",
                    pool,
                    len(txo_list),
                )
                await self.preallocate_txo_pool(pool, txo_list, off_limits=utxos)
            utxos.update(await self.get_txos_from_pool(pool, txo_list))
        return utxos

    async def record_tx(
        self,
        entrant: str,
        reason: str,
        amount: int,
        direction: str,
        wallet_address: str,
        entry_block: int,
    ) -> None:
        """
        Record transaction into database to be recovered in case of bot failure
        """

        data = dict(
            username=entrant,
            wallet_address=wallet_address,
            amount=amount,
            direction=direction,
            tx_purpose=reason,
            tx_block=entry_block,
            bot_number=self.bot_number,
        )

        logging.info(
            "recording %s - %s:%s:%s in DB", direction, entrant, amount, reason
        )
        logging.info("airdrop UUID: %s", self.config.name)
        await self.db_manager.sd_add_tx(self.config.name, json.dumps(data))

    async def get_state(self) -> set[States]:
        """
        get airdrop state based on environment

        Returns:
          set: Set of state
        """
        states = set()
        conf = self.config
        num_entrants = len(self.entrant_list)
        paid_entrants = self.get_entrants("paid")
        num_paid_entrants = len(paid_entrants)
        if isinstance(conf, SimpleAirdrop):
            if conf.max_entrants > 0:
                if num_paid_entrants >= num_entrants >= conf.max_entrants:
                    states.add(States.AIRDROP_FINISHED)
                    return states
            if not conf.is_configured_correctly():
                states.add(States.SETUP)
            else:
                wallet_balance = await self.mobster.get_wallet_balance()
                if conf.is_ready_to_launch(wallet_balance):
                    if conf.start_block > 0:
                        states.add(States.LIVE)
                    else:
                        if num_entrants > 0:
                            states.add(States.ERROR)
                            return states
                        states.add(States.SETUP)
                        states.add(States.READY_TO_LAUNCH)
                else:
                    if num_entrants > 0 or conf.start_block > 0:
                        states.add(States.LIVE)
                        states.add(States.NEEDS_FUNDING)
                        states.add(States.ERROR)
                        logging.critical(
                            "Airdrop is live without funding, please fund wallet immediately!!"
                        )
                    else:
                        states.add(States.SETUP)
                        states.add(States.NEEDS_FUNDING)
                if num_entrants >= conf.max_entrants:
                    states.add(States.AIRDROP_FULL)
        else:
            states.add(States.NO_AIRDROP)
        logging.debug("state are %s", states)
        return states

    async def distribute_drop(self, num_to_drop: int, max_retries: int = 2) -> None:
        """
        Executes payment of airdrop amount to recipients
        """
        config = self.config
        assert isinstance(config, SimpleAirdrop)
        unpaid = self.get_entrants("unpaid")
        unpaid.extend(self.get_entrants("failed"))
        if num_to_drop > len(unpaid) or num_to_drop < 1:
            msg = f"drop specfied for {num_to_drop}/{len(unpaid)} users, aborting "
            logging.warning(msg)
            await self.send_message(get_secret("ADMIN"), msg)
            return

        drop_amount = config.to_pmob(config.drop_amount)
        unpaid = unpaid[0:num_to_drop]
        network_block = await self.mobster.get_current_network_block()
        for entrant in unpaid:
            logging.info("Attempting to pay %s", entrant.get("username"))
            entrant_address = await self.get_address(entrant["username"])
            if not isinstance(entrant_address, str):
                logging.warning(
                    "could not get payment address for %s, skipping",
                    entrant.get("username"),
                )
                continue
            async with self.resource_lock:
                pool = await self.get_txos_from_pool("drop", [drop_amount])
            logging.debug(
                "Attempting to pay %s with wallet: %s with txos: %s",
                entrant.get("username"),
                entrant_address,
                pool,
            )
            payment_message = (
                "Here's your airdrop! I've just sent you "
                f"{config.drop_amount} MoB! Thank you so much "
                "for participating!"
            )

            result = await self.send_payment(
                entrant["username"],
                drop_amount,
                payment_message,
                30,
                comment="airdrop_payout",
                input_txo_ids=list(pool.keys()),
            )
            logging.info("payment result is %s", result)

            if isinstance(result, Message) and result.status == "tx_status_succeeded":
                create_task(
                    self.record_tx(
                        entrant["username"],
                        "airdrop_payout",
                        drop_amount,
                        "tx_direction_sent",
                        entrant_address,
                        network_block,
                    )
                )
                self.entrant_list[entrant["username"]]["status"] = "paid"
            else:
                self.send_message(
                    entrant["username"],
                    "There was an issue with your payment, I'll try again soon!",
                )
                logging.info(
                    "Airdrop failed to %s failed with address %s ",
                    entrant["username"],
                    entrant_address,
                )
                self.entrant_list[entrant["username"]]["status"] = "failed"
                continue

        drop_stats = await self.get_drop_stats()
        create_task(
            self.send_message(
                get_secret("ADMIN"), "Drop successful! Here are the stats on the drop:"
            )
        )
        create_task(self.send_message(get_secret("ADMIN"), drop_stats))

    @hide
    @requires_admin
    async def do_message(self, msg: Message) -> Response:
        """
        Send message to a specified user
        """
        if msg.tokens and len(msg.tokens) > 1:
            message = " ".join(msg.tokens[1:])
            create_task(self.send_message(msg.tokens[0], message))
            return f"Message sent to {msg.tokens[0]}"
        else:
            return (
                "This command sends a message to a user\n\nUsage: "
                "/message +15555555555 [your message]"
            )

    async def reconcile_data(self, start_block: int) -> dict[str, list]:
        """
        Returns any discrepancies that exist in the airdrop bot's payment
        records and full service payment records. If "mismatch_paid" records
        exist, it indicates that the bot has recorded payments which weren't
        actually made. "Mismatch_unpaid" indicates some entrants haven't been
        tracked by the bot.

        args:
          start_block (int): Block from which to measure discrepancies

        Returns:
          dict(str, set): List of mismatches in paid and unpaid entrants
        """
        config = self.config
        assert isinstance(config, SimpleAirdrop)
        paid = set(entrant["wallet_address"] for entrant in self.get_entrants("paid"))
        unpaid = set(
            entrant["wallet_address"] for entrant in self.get_entrants("unpaid")
        )
        fs_entrants = await self.get_entrants_from_full_service(start_block)
        fs_paid = set(fs_entrants["paid_entrants"])
        fs_unpaid = set(fs_entrants["unpaid_entrants"])
        paid_diff = list(paid - fs_paid)
        unpaid_diff = list(unpaid - fs_unpaid)
        return {"mismatch_paid": paid_diff, "mismatch_unpaid": unpaid_diff}

    async def get_entrants_from_full_service(self, start_block: int) -> dict[str, list]:
        """
        Get full service record of which entrants have been entered and/or have
        been paid.

        args:
          start_block (int): block from which to measure from

        Returns:
          dict[str, list]: list of paid and unpaid entrants within the block
          specified
        """
        account_id = await self.mobster.get_account()
        record_list = await self.mobster.get_transactions(account_id)
        _id = "recipient_address_id"
        entrants: set = set()
        paid_entrants: set = set()
        for _, record in record_list.items():
            sent_block = try_cast_int(record.get("finalized_block_index"))
            otxos = record.get("output_txos")
            if (
                isinstance(sent_block, int)
                and sent_block >= start_block
                and record.get("direction") == "tx_direction_sent"
                and isinstance(otxos, list)
            ):
                for txo in otxos:
                    if (
                        record.get("comment")
                        in [
                            "prepaid_airdrop_entry",
                            "airdrop_entry",
                        ]
                        and txo.get(_id)
                    ):
                        entrants.add(txo.get(_id))
                    if record.get("comment") == "airdrop_payout" and txo.get(_id):
                        paid_entrants.add(txo.get(_id))
        unpaid_entrants: set = entrants - paid_entrants
        return {
            "entrants": list(entrants),
            "paid_entrants": list(paid_entrants),
            "unpaid_entrants": list(unpaid_entrants),
        }

    def get_entrants(self, status: str = "unpaid", num_only: bool = False) -> list:
        """
        Get entrants based on paid or unpaid status

        args:
          status (str): either "paid" to get entrants who have been sent mob or
          "unpaid" to get entrants who have yet to receive an airdrop
        """
        entrants = self.entrant_list
        if num_only:
            return [k for k, v in entrants.items() if v.get("status") == status]
        return [v for _, v in entrants.items() if v.get("status") == status]

    @hide
    @requires_admin
    async def do_make_drop(self, msg: Message) -> Response:
        """
        Initiate Airdrop
        """
        state = await self.get_state()
        if States.SETUP in state:
            return "Airdrop currently in setup, please finish setup"
        if States.ERROR in state:
            return "Airdrop is in error state, please correct error before continuing"
        if States.NO_AIRDROP in state:
            return (
                "No Airdrop active, please configure and launch one before continuing"
            )
        if States.LIVE in state and isinstance(self.config, SimpleAirdrop):
            if States.NEEDS_FUNDING in state:
                amount_needed = self.config.get_total_airdrop_amount()
                resp = f"Funding needed, ensure wallet has at least {amount_needed} MoB"
                logging.warning(resp)
                return resp
            unpaid_entrants = self.get_entrants()
            if len(unpaid_entrants) <= 0:
                prepaid = len(self.get_entrants("prepaid"))
                if prepaid > 0:
                    return (
                        "No fully registered users currently but "
                        f"here are {prepaid} users who have completed "
                        "the prepay step but have not yet completed full "
                        "registration for the airdrop by sending back the "
                        "initial MoB. You can type /blast unpaid [your message] "
                        "to remind them!"
                    )
                return "There are no users who have fully registered yet! Type /drop_stats to show registered users"
            data = {"num_unpaid_entrants": len(unpaid_entrants)}
            self.config.enter_dialog(Dialog.ADMIN_DISTRIBUTE_MOBILECOIN)
            return self.config.get_next_dialog(data=data)
        if States.AIRDROP_FINISHED:
            return "Airdrop finished, cannot make further drops, please type /finish_airdrop to cleanup airdrop"
        return "Not able to make drop"

    @hide
    @requires_admin
    async def do_blast(self, msg: Message) -> Response:
        recipient_list = []
        logging.info(msg)
        valid = ["all", "paid", "entered", "prepaid"]
        if msg.tokens and msg.tokens[0] in valid:
            if len(msg.tokens) == 1:
                return "You made a valid selection but didn't type a message!"
            if msg.tokens[0].lower() == "all":
                recipient_list += list(self.entrant_list.keys())
            if msg.tokens[0].lower() == "entered":
                recipient_list += self.get_entrants("unpaid", num_only=True)
            if msg.tokens[0].lower() in ["paid", "prepaid"]:
                recipient_list += self.get_entrants(msg.tokens[0], num_only=True)
            if len(recipient_list) <= 0:
                if msg.tokens[0] == "all":
                    return "No users yet to message!"
                else:
                    return f"No {msg.tokens[0]} users currently!"
            else:
                blast = " ".join(msg.tokens[1:])
                for user in recipient_list:
                    await asyncio.sleep(0.2)
                    create_task(self.send_message(user, blast))
                return f"Your message was sent to {len(recipient_list)} users!"
        default = (
            "To use this command type /blast [users] [message]\n\n"
            "/blast all will mesage all users\n"
            "/blast paid will message users who have already been paid\n"
            "/blast entered will message users who are registered for the airdrop\n"
        )
        if isinstance(self.config, SimpleAirdrop) and self.config.entry_price > 0:
            default += "/blast prepaid will message users who have been sent initial mob but not fully entered"
        return default

    @hide
    @requires_admin
    async def do_drop_stats(
        self, msg: Message
    ) -> Response:  # pylint: disable=unused-argument
        """
        Get stats on airdrop
        """
        return await self.get_drop_stats(msg)

    async def get_drop_stats(self, msg: Optional[Message] = None) -> Response:
        """
        Provide data on airdrop status
        """
        state = await self.get_state()
        if States.NO_AIRDROP in state:
            return "Sorry, no airdrop in progress"
        conf = self.config
        num_entrants = len(self.entrant_list)
        prepaid = self.get_entrants("prepaid", num_only=True)
        paid = self.get_entrants("paid", num_only=True)
        entered = self.get_entrants("unpaid", num_only=True)
        failed = self.get_entrants("failed", num_only=True)
        wallet_balance = await self.mobster.get_wallet_balance()
        assert isinstance(conf, SimpleAirdrop)
        if wallet_balance >= 0:
            finances = f"Wallet Balance: {wallet_balance}"
        else:
            finances = "Wallet Balance: Error getting balance!"
        resp = f"{finances}\n" + f"{repr(conf)}"
        if States.LIVE in state:
            prefix = "Airdrop live\n"
            prefix += "You can type /make_drop at anytime to make a drop\n\n"
            prefix += f"# of entrants: {num_entrants}\n"
            if conf.entry_price > 0:
                prefix += f"# of entrants given starter MoB: {len(prepaid)}\n"
            prefix += (
                f"# of entrants entered: {len(entered)}\n"
                f"# of entrants paid in full: {len(paid)}\n"
                f"# of entrants for which airdrop payment failed {len(failed)}\n"
            )
            if isinstance(msg, Message) and msg.text:
                if "showusers" in msg.text.lower():
                    if conf.entry_price > 0:
                        prefix += f"entrants given starter MoB: {prepaid}\n"
                    prefix += (
                        f"entrants entered: {entered}\n"
                        f"entrants paid in full: {paid}\n"
                        f"entrants with payment failures: {failed}\n"
                    )
                if "utxos" in msg.text.lower():
                    prefix += "\nTxo Pools:\n"
                    for pool, utxos in self.txo_pools.items():
                        prefix += f"{pool}: {utxos}\n"
            if States.NEEDS_FUNDING in state:
                warn = "CRITICAL: AIRDROP HAS ENTRANTS BUT LACKS FUNDING, FUND IMMEDIATELY\n"
                prefix = warn + prefix
            if States.AIRDROP_FULL in state:
                warn = "AIRDROP FULL!\n"
                prefix = warn + prefix
            return prefix + resp
        if States.AIRDROP_FINISHED in state:
            prefix = (
                "Airdrop Finished! "
                f"{num_entrants}/{len(paid)} entrants paid "
                f"{conf.drop_amount} MoB. Type /finish_airdrop to cleanup drop\n"
            )
            return prefix + resp
        if States.SETUP in state:
            setup_done = conf.is_configured_correctly()
            prefix = (
                "Setup is currently in progress\n"
                f"Setup Complete: {setup_done}\n"
                f"Fully Funded: {not (States.NEEDS_FUNDING in state)}\n"
                f"Able to Launch: {States.READY_TO_LAUNCH in state}\n"
            )
            return prefix + resp

        return "Drop in Error State\n" + f"# of entrants {num_entrants}\n" + resp

    @hide
    @requires_admin
    async def do_launch_airdrop(
        self, msg: Message
    ) -> Response:  # pylint: disable=unused-argument
        """
        Put airdrop in state to accept funds from users
        """
        resp = "No airdrop configured, cannot launch"
        conf = self.config
        state = await self.get_state()
        if States.LIVE in state:
            return "Airdrop already in progress, cannot launch new airdrop"

        if isinstance(conf, SimpleAirdrop) and States.SETUP in state:
            if States.READY_TO_LAUNCH in state:
                block_height = await self.mobster.get_current_network_block()
                if block_height < 0:
                    return "Network block height couldn't be found, aborting launch"
                account = await self.mobster.get_account()
                drop_txos = [conf.to_pmob(conf.drop_amount)] * (
                    conf.max_entrants - len(self.txo_pools.get("drop", []))
                )
                entry_txos = [conf.to_pmob(conf.entry_price) + FEE_PMOB] * (
                    conf.max_entrants - len(self.txo_pools.get("entry", []))
                )
                start_amount = sum(drop_txos) + sum(entry_txos)
                locked_txos = self.txo_pools.get("drop", {}).copy()
                locked_txos.update(self.txo_pools.get("entry", {}).copy())
                await self.send_message(msg.source, "Attempting to launch the airdrop!")
                await self.send_message(
                    msg.source,
                    (
                        "Please hold on a second while I ensure "
                        "our finances are organized, this might take a minute!"
                    ),
                )
                unlocked_balance = await self.mobster.get_filtered_balance(locked_txos)
                if unlocked_balance < start_amount:
                    resp = "Airdrop launch failed\n\n"
                    resp += f"Deficit of {unlocked_balance - start_amount} exists "
                    resp += "please try /cleanup_wallet and try again. If this "
                    resp += "doesn't work please fund wallet with "
                    resp += f"{start_amount- unlocked_balance} MoB"
                    return resp
                name = f"simple_airdrop_{block_height}_{datetime.utcnow().strftime('%Y_%m_%d_%H')}"
                async with self.resource_lock:
                    await self.preallocate_txo_pool("drop", drop_txos)
                async with self.resource_lock:
                    await self.preallocate_txo_pool("entry", entry_txos)
                txos_drop = len(self.txo_pools.get("drop", []))
                txos_entry = len(self.txo_pools.get("entry", []))
                if txos_drop >= conf.max_entrants and txos_entry >= conf.max_entrants:
                    conf.set_airdrop(block_height, name)
                    await self.db_manager.create_simple_airdrop(
                        name, block_height, "simple", json.dumps(conf.asdict())
                    )
                    resp = (
                        f"Airdrop launched at block: {block_height}\n{self.bot_number} "
                    )
                    resp += f"will now accept payments. "
                    resp += "You can now announce this airdrop!\n\n"
                    resp += "You can check the status of this airdrop anytime by typing /drop_stats. "
                    resp += (
                        "When you're ready to make a drop users who have entered, type "
                    )
                    resp += "/make_drop and you'll be able to select how many users to drop to!\n\n"
                    resp += "To cancel this airdrop, type /cancel_airdrop "
                    return resp
                amount_to_add = max(0, conf.max_entrants - txos_drop) * (
                    conf.drop_amount + FEE_MOB
                ) + max(0, conf.max_entrants - txos_entry) * (
                    conf.entry_price + FEE_MOB
                )
                return (
                    "Airdrop launch failed\n\nTry running "
                    "/cleanup_wallet and then try launching again. "
                    f"If this doesn't work, add {amount_to_add} "
                    "MoB in a single transaction to ensure "
                    "proper txo_allocation"
                )
            if States.NEEDS_FUNDING in state:
                funds = await self.mobster.get_wallet_balance()
                deficit = conf.get_total_airdrop_amount() - funds
                return f"Airdrop needs funding, please fund with {deficit} MoB"
            return "Airdrop setup not complete, type /setup_airdrop to finish setup"

        if States.ERROR in state:
            resp = "Airdrop is in an error state, please fix before launching"

        return resp

    async def check_last_airdrop(self, notify: str = "") -> Response:
        if not notify:
            notify = get_secret("ADMIN")
        msg = (
            "There is a previous airdrop which was not finished, if you'd "
            "like to recover it type /recover_airdrop at anytime to restart it "
        )
        record = await self.db_manager.sd_get_last_airdrop()
        if isinstance(record[0].get("drop_config"), str):
            last_record = json.loads(record[0].get("drop_config"))
            if last_record.get("end_state") != "finished":
                await self.send_message(notify, msg)
            return last_record.get("end_state")
        return "no_previous_airdrop"

    @hide
    @requires_admin
    async def do_setup_airdrop(
        self, msg: Message
    ) -> str:  # pylint: disable=unused-argument
        """
        Setup new airdrop or re-enter in progress airdrop
        """
        state = await self.get_state()
        await self.check_last_airdrop(msg.source)
        if States.NO_AIRDROP in state:
            self.config = SimpleAirdrop()
            assert isinstance(self.config, SimpleAirdrop)
            self.config.enter_dialog(Dialog.ADMIN_AIRDROP_SETUP)
            return self.config.get_next_dialog()
        if States.LIVE in state:
            return "An airdrop is in progress, cannot setup a new one"
        if States.ERROR in state:
            return "Airdrop is in error state, cannot enter setup"
        if States.SETUP in state:
            assert isinstance(self.config, SimpleAirdrop)
            self.config.enter_dialog(Dialog.ADMIN_AIRDROP_SETUP)
            return self.config.get_next_dialog()
        return "Unknown Error, cannot setup airdrop"

    @hide
    @requires_admin
    async def do_cancel_airdrop(self, msg: Message) -> Response:
        """
        Cancel airdrop and reset airdrop data
        """
        state = await self.get_state()
        resp = "Airdrop being cancelled, you can restart one at anytime"
        force = False
        logging.info(msg)
        if msg.text and "now!" in msg.text.lower():
            force = True
        if States.AIRDROP_FINISHED in state:
            return "Airdrop should be finished with /finish_airdrop command"
        if States.LIVE in state:
            if not force:
                resp = "Warning an airdrop is active! Type '/cancel_airdrop "
                resp += "NOW!' to end it without paying first\n\n"
                resp += "If you do cancel you can type /recover_airdrop to recover it at anytime"
                return resp
            resp = "Airdrop Cancelled!"
        if States.SETUP in state:
            resp = (
                "Airdrop configuration cancelled, you can configure another at anytime"
            )
        if isinstance(self.config, SimpleAirdrop):
            self.config.set_end_state("cancelled")

        self.config = Airdrop()
        self.entrant_list = {}
        self.txo_pools = {"deadpool": {}}
        return resp

    @hide
    @requires_admin
    async def do_finish_airdrop(self, msg: Message) -> Response:
        """
        Cleanup Airdrop
        """
        state = await self.get_state()
        if States.AIRDROP_FINISHED in state:
            if isinstance(self.config, SimpleAirdrop):
                self.config.set_end_state("finished")
                await self.db_manager.sd_update_config(
                    self.config.name, json.dumps(self.config.asdict())
                )
            self.config = Airdrop()
            self.entrant_list = {}
            self.txo_pools = {"deadpool": {}}
            return "All airdrop data cleared, you may start another at anytime"
        if States.LIVE in state:
            return "There are still unpaid entrants in the list, please payout remaining entrants before finishing airdrop"
        if States.SETUP in state:
            return "Airdrop is still in setup, can't finish airdrop, type /cancel_airdrop if you'd like to cancel the drop"
        return "No airdrop is currently in progress"

    async def cleanup_wallet(self, clean_all: bool = False) -> None:
        """
        Squashes transactions in wallet so tey can be allocated correctly
        """
        account_id = await self.mobster.get_account()
        locked_txos: dict[str, Any] = {}
        balance = await self.mobster.get_filtered_balance(locked_txos)
        if not clean_all:
            for pool, utxos in self.txo_pools.items():
                locked_txos.update(utxos)
        unlocked_utxos = await self.mobster.filter_utxos(
            account_id, locked_txos=locked_txos
        )
        cleanup_amount = balance - (round(len(unlocked_utxos) / 16) + 5) * FEE_PMOB
        await self.mobster.cleanup_utxos(
            account_id, cleanup_amount, locked_txos=locked_txos
        )

    @hide
    @requires_admin
    async def do_cleanup_wallet(self, msg: Message) -> Response:
        notify = "Wallet cleanup started, I'll send you a message "
        notify += "when it completes. Please wait until it completes "
        notify += "before making any drops or payments"
        await self.send_message(msg.source, notify)
        clean_all = False
        if msg.text and "cleanall" in msg.text:
            clean_all = True
        async with self.resource_lock:
            await self.cleanup_wallet(clean_all=clean_all)
        return "Wallet cleanup completed, you can resume normal operations!"


if __name__ == "__main__":

    @app.on_startup.append
    async def start_wrapper(out_app: web.Application) -> None:
        """
        Start App
        """

        out_app["bot"] = AirDropBot()

    web.run_app(app, port=8080, host="0.0.0.0", access_log=None)
