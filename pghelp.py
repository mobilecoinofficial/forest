...
LockPGExpressions = PGExpressions(
    table="",
    create_table="Not Applicable;",
    grab_fs_blocking="SELECT pg_advisory_lock($1);",
    grab_fs_nonblocking="SELECT pg_try_advisory_lock($1);",
    release_fs="SELECT pg_advisory_unlock($1);",
)


class LockManager(PGInterface):
    """Abstraction for PG Advisory Locks"""

    def __init__(self, queries=LockPGExpressions, database="", loop=None) -> None:
        super().__init__(queries, database, loop)

    @staticmethod
    def str_int_expand(x: str) -> int:
        return int.from_bytes(hashlib.sha1(x.encode()).digest(), "little") % 0x7FFFFFFF

    def lock(self, name: str, caller: str = "") -> bool:
        current_frame = inspect.currentframe()
        if not caller and current_frame:
            calling_f_name = inspect.getframeinfo(current_frame.f_back).function
        else:
            calling_f_name = caller
        res: List[asyncpg.Record] = self.sync_grab_fs_blocking(
            self.str_int_expand(f"{calling_f_name}_{name}")
        )
        return res[0].get("pg_advisory_lock")

    def nonblock_lock(self, name: str, caller: str = "") -> bool:
        current_frame = inspect.currentframe()
        if not caller and current_frame:
            calling_f_name = inspect.getframeinfo(current_frame.f_back).function
        else:
            calling_f_name = caller
        res: List[asyncpg.Record] = self.sync_grab_fs_nonblocking(
            self.str_int_expand(f"{calling_f_name}_{name}")
        )
        return res[0].get("pg_try_advisory_lock")

    def unlock(self, name: str, caller: str = "") -> bool:
        current_frame = inspect.currentframe()
        if not caller and current_frame:
            calling_f_name = inspect.getframeinfo(current_frame.f_back).function
        else:
            calling_f_name = caller
        res: List[asyncpg.Record] = self.sync_release_fs(
            self.str_int_expand(f"{calling_f_name}_{name}")
        )
        return res[0].get("pg_advisory_unlock")
