...
class ScheduleBot(QuestionBot):
    ...
    def parse_schedule(self, message: Message) -> tuple[maya.MayaDT,str]:
        """Parse a message of the form schedule "yyyy-mm-dd HH:MM" "message" """

        try:
            outgoing_time = maya.when(message.arg1)
        except ValueError:
            logging.info("couldn't get time automatically")
    ...
    async def do_schedule(self, message: Message) -> str:
        ...
        if await self.ask_yesno_question(
            message.source,
            f"Ok we'll send \n {outgoing_message} \n in {time_until.seconds} seconds. \n Is that Ok? do you like this, am I a gfood bot y/n",
        ):
            asyncio.create_task(
                self.schedule_send_message(
                    message.source, outgoing_message, outgoing_time
                )
            )
