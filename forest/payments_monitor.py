...
class FullService:
    """
    Async Python wrapper around Mobilecoin full service API
    """
    ...
    async def import_account(self, name: str = "default", mnemonic: str = "") -> dict:
        """
        Import account from 24 word account mnemonic/entropy
    ...
    async def get_all_txos_for_account(self, account_id: str = "") -> dict:
        """
        Get all transaction outputs (txos) for account
...
class Mobster(FullService):
    ...
    async def get_all_utxos(
        self, account_id: str = "", locked_txos: Optional[Union[list, dict]] = None
    ) -> dict:
        if not account_id:
            account_id = await self.get_account()
        return await self.get_all_txos_matching_status(
            account_id, "txo_status_unspent", locked_txos=locked_txos
        )

    async def get_all_txos_matching_status(
        self,
        account_id: str = "",
        txo_status: str = "txo_status_unspent",
        locked_txos: Optional[Union[list, dict]] = None,
    ) -> dict:
        """
        Get all transaction outputs (utxos) for account matching a specific
        status.

        args:
          account_id (str): mobilecoin account id
          locked_txos (Union[list, dict]): optional txo ids to exclude from result
